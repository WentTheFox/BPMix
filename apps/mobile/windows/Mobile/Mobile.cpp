...
// Native module methods run on a background dispatch thread, not the UI
// thread, so GetActiveWindow()/GetForegroundWindow() can't find our window
// from there - it's captured once here, on the UI thread, instead.
