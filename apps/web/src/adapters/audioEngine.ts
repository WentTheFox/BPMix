    ...
    createSource(audio: DecodedAudio, onEnded?: () => void): SourceNode {
      ...
      const node: SourceNode = {
        ...
        setGain(value) {
          ...
          // at the current time, which real browsers throw NotSupportedError
          // for if it lands inside an already-scheduled setValueCurveAtTime()
          // window - and rampGainCurve() below (a crossfade's fade curve)
          // schedules exactly that, several seconds ahead of when it
          // actually starts. A volume-slider tweak or normalization-gain
          // update landing inside that window is routine, not exceptional -
          // previously uncaught, this crashed the whole app. Dropping the
          // update is safe: the curve already governs gain until it
          // finishes, so the dropped value has no audible effect anyway;
          // the next call after the curve completes goes through normally.
          try {
